...
async function generateTableInterfaces(connection: mysql.PoolConnection) {
    ...
    for (const table of tables as any[]) {
        ...
        (columns as any[]).forEach((column: any) => {
            interfaceContent += `  ${formatColumnName(column.COLUMN_NAME)}: ${mapMySQLTypeToTypescript(column.COLUMN_NAME, column.DATA_TYPE, enumMapping)};\n`;
        });
...
function formatColumnName(columnName: string): string {
    return removeMultiLingualStringSuffix(columnName);
}
...
function mapMySQLTypeToTypescript(columnName: string, mysqlType: string, enumMapping: Record<string, string>): string {
    ...
    if (isMultiLingualString(columnName))
...
export function isMultiLingualString(columnName: string): boolean {
